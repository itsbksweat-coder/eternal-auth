...
const backup=path.join('backups',`before-1.8.9-${new Date().toISOString().replace(/[:.]/g,'-')}.sql`);
...
run(['d1','execute',db.database_name,'--remote','--file=./migrations/v1.8.9-panel-scoped-licenses.sql']);
run(['deploy']);
console.log('Deployment command succeeded. Sync Discord commands from the dashboard, then open /api/health and test the dashboard and bot.');
