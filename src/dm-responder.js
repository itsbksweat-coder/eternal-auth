...
export class DmResponder {
  ...
  async handle(message) {
    ...
    try {
      ...
      const response = await this.request(`https://discord.com/api/v10/channels/${message.channel_id}/messages`, {
        method: 'POST',
      ...
      if (!response.ok) { this.lastError = `Discord DM HTTP ${response.status}`; return false; }
      this.sent++;
      this.lastError = null;
      return true;
    } catch {
