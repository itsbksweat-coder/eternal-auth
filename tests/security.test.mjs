...
test('client security reports are informational and never create HWID bans', async () => {
  ...
  for (const reason of ['gui','clipboard','file','console','network','integrity','environment','hwid_spoof']) {
...
test('clipboard source leak persistently HWID-blacklists authenticated client', async () => {
  ...
  assert.equal(body.status,'Blacklisted');
...
test('FFA is keyless, requires a device, respects the switch and returns protected source', async () => {
  ...
  assert.doesNotMatch(bootstrap,/__ea_block|hookmetamethod/);
...
test('FFA non-spy signed reports remain informational', async () => {
  const {db,env}=await fixture();
